<template>
	<view v-if="scanFinish" style="display: flex;flex-direction: column;justify-content: center;align-items: center;">
	
		<view style="font-size: 36rpx;"><text>{{$t("scanResult")}}<!-- 扫码结果 --></text></view>
		...
		methods: {
			...
			onmarked(type, result) {
				...
				if(result.indexOf("http://")>-1 || result.indexOf("https://")>-1){
					uni.redirectTo({
						url:"/pages/index/external_view?url="+result
					})
				}else{
